...
wget -q $GITURL/$ENABLE_SCRIPT -O /root/GPS/$ENABLE_SCRIPT
wget -q $GITURL/$DISABLE_SCRIPT -O /root/GPS/$DISABLE_SCRIPT
wget -q $GITURL/$GPS_PY -O /root/GPS/$GPS_PY

if [ ! -f /root/GPS/$ENABLE_SCRIPT ] || \
   [ ! -f /root/GPS/$DISABLE_SCRIPT ] || \
   [ ! -f /root/GPS/$GPS_PY ]; then
...
# Insert the two DTMF commands immediately after the stanza header
